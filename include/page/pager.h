...
inline bool Pager::Open(const std::string &file_name)
{
...
inline void Pager::Close()
{
    if (file_id_) { PageFileSystem::GetInstance().Close(file_id_); }
    file_id_ = 0;
}

inline void Pager::Flush()
{
    PageFileSystem::GetInstance().WriteBack(file_id_);
}

inline void Pager::MarkDirty(int page_id)
{
    PageFileSystem::GetInstance().MarkDirty(file_id_, page_id);
}

inline int Pager::NewPage()
{
    return PageFileSystem::GetInstance().Allocate(file_id_);
}

inline void Pager::FreePage(int page_id)
{
    PageFileSystem::GetInstance().Deallocate(file_id_, page_id);
}

inline char *Pager::Read(int page_id)
{
    return PageFileSystem::GetInstance().Read(file_id_, page_id);
}

inline char *Pager::ReadForWrite(int page_id)
{
